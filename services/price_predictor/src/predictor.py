...
class Predictor:
    ...
    def from_model_registry(
        ...
        # - features_to_use: List[str],
        # - prediction_window_sec: int,
        ohlc_window_sec = int(experiment.get_parameters_summary('ohlc_window_sec')['valueCurrent'])
        ...
        # TODO: last_n_minutes is a parameter that you should log in the experiment when
        # you train the model. I used a hardcoded value here, but you should get it from the experiment
        last_n_minutes = 30
        ...
        prediction_window_sec = int(experiment.get_parameters_summary('prediction_window_sec')['valueCurrent'])
    ...
    def predict(self) -> PredictorOutput:
        ...
        logger.debug('Preprocessing the data - keeping features in `self.features_to_use`')
        ...
        current_ts = pd.to_datetime(current_ts_ms, unit='ms').strftime('%Y-%m-%d %H:%M:%S')
        predicted_ts = pd.to_datetime(predicted_ts_ms, unit='ms').strftime('%Y-%m-%d %H:%M:%S')
...
if __name__ == '__main__':
    ...
    #     # TODO: I added this large number to make sure I get some
    #     # data from the online store.
    #     # At the moment, my feature pipeline is up and running, and
    #     # successfully sending data to Hopsworks.
    #     # But, Hopsworks (for some unkown reason to me) is not
    #     # pushing this data to the online store.
    #     # I just asked on Slack for help. Once I get this fixed,
    #     # I will revert this to 30.
    #     last_n_minutes=30, #60*24*7
    #     features_to_use=[
    #         'rsi',
    #         'momentum',
    #         'std',
    ...
    logger.debug('Running the Predictor class with the from_model_registry class method')
